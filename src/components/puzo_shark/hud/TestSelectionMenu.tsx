...
const kidBack: CSSProperties = {
  ...
  background: 'rgba(0,210,255,0.15)', color: '#d6f6ff',
...
const CARD_BGS = [
  'linear-gradient(135deg, rgba(0,210,255,0.35) 0%, rgba(0,120,255,0.14) 100%)',
  'linear-gradient(135deg, rgba(244,114,182,0.35) 0%, rgba(168,85,247,0.14) 100%)',
  'linear-gradient(135deg, rgba(251,191,36,0.35) 0%, rgba(245,158,11,0.14) 100%)',
  'linear-gradient(135deg, rgba(45,212,191,0.35) 0%, rgba(13,148,136,0.14) 100%)'
];
        ...
        <div key={currentSubject.title} style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 6px 16px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 235px), 1fr))', gap: 12, alignContent: 'start' }}>
          {currentSubject.tests.map((t, i) => (
            <button key={t.title + i} onClick={() => startGame(t)} style={{ ...testCard, background: CARD_BGS[i % CARD_BGS.length] }}>
              ...
              <span style={{ fontSize: '0.85rem', opacity: 1, flexShrink: 0 }}>{t.questions.length} в.</span>
            </button>
          ))}
        </div>
      </div>
