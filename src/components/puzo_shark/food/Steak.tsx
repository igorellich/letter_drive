import { useRef, useMemo, useState, useEffect, useContext, Suspense } from "react"
import { useFrame } from "@react-three/fiber"
import { Center, PositionalAudio, useGLTF } from "@react-three/drei"
...
export const Steak: React.ComponentType<{ item: FoodItem, onSelectAnswer:(item: FoodItem)=>void }> = (props: { item: FoodItem, onSelectAnswer:(item: FoodItem)=>void }) => {
  ...
  useEffect(() => {
    ...
    if(brokenBoxSoundRef.current){
      brokenBoxSoundRef.current.play();
    }
  }, [showSteak])
  useEffect(() => {
